...
class Index(abc.ABC):
    ...
    @encoder.setter
    def encoder(self, encoder: TCTColBERTQueryEncoder) -> None:
        """Set the query encoder.

        Args:
            encoder (QueryEncoder): The encoder.
        """
        assert encoder is None or isinstance(encoder, TCTColBERTQueryEncoder)
    ...
    def get_scores(
        ...
        t0 = time.time()
        print("Encoding queries for interpolation...")
        ...
        if not early_stopping:
            ...
            for a in alpha:
                result[a] = interpolate(
                    ranking, Ranking(dense_run, sort=False), a, sort=True, normalise=normalise
                )
